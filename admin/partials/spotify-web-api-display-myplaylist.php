 <?php
	...
	$playlist_grid = get_option('_number_grid_playlist');
	$bg_box_playlist = get_option('_bg_box_playlist');
 ...
 <style>
 	...
 	.bg-box {
 		background-color: <?= $bg_box_playlist ? $bg_box_playlist : '#fff'; ?>;
 ...
 <div class="flex flex-1 flex-col bg-box gap-2 sm:-mb-4" style="max-width: inherit;">
 	<div class="flex flex-col justify-center items-center my-5">
 		<form action="#" id="" method="POST" data-url="<?= admin_url('admin-ajax.php'); ?>">
 			<input type="text" class="form-search" name="search_top" id="search_top" placeholder="Search Playlist" />
 			<input type="hidden" name="action" id="search_chart" value="search_chart" />
 		</form>
 	</div>
 	<div class="flex flex-1 flex-row">
 		<div class="grid grid-cols-2 lg:grid-cols-<?= $playlist_grid ? $playlist_grid : '4' ?> mx-auto gap-4" id="frame">
 			<?php foreach ($myPlaylist->items as $top_track) : ?>
 				<a href="#data_playlist" onclick="ShowAnyTracks(this)" data-name="<?= $top_track->name; ?>" data-id="<?= $top_track->id; ?>" class="flex flex-col hover:bg-transparent focus:bg-transparent">
 					<img src="<?= $top_track->images[0]->url ? $top_track->images[0]->url : plugin_dir_url('spotify-web-api/admin/images/') . 'no_image.jpg' ?>" class="lg:w-40 md:h-40 lg:w-40 md:h-40 sm:w-36 sm:h-36" />
 ...
 <script>
 	...
 	function ShowAnyTracks(event) {
 		...
 		$playlist = `
				<iframe style="border-radius:12px" src="https://open.spotify.com/embed/playlist/${data_id}?utm_source=generator" width="100%" height="380" frameBorder="0" allowfullscreen="" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture"></iframe>
				`
 		$('#playlist').fadeIn().html($playlist)
 		$.ajax({
 			...
 			$.ajax({
 				...
 				success: function(data) {
 					...
 					if (search_top !== '') {
 						...
 						for (var i = 0; i < search.playlist.length; i++) {
 							let obj = search.playlist[i]
 							data_array.push(obj);
 						}
 						...
 						$.each(result_search, function(index, val) {
 							if (val) {
 								let frame = `<button type="button" onclick="PlayTrack(this)" data-id="${val.id}" class="flex flex-col hover:bg-transparent focus:bg-transparent text-center">
														<img src="${val.images[0] !== undefined? val.images[0].url :''}" class="w-40 h-40" />
														<p>${val.name}</p>
												</button>`
 						...
 						for (var i = 0; i < search.playlist.length; i++) {
 							...
 							let all_frame = `<button type="button" onclick="PlayTrack(this)" data-id="${obj.id}" class="flex flex-col hover:bg-transparent focus:bg-transparent text-center">
														<img src="${obj.images[0] !== undefined ? obj.images[0].url :''}" class="w-40 h-40" />
														<p>${obj.name}</p>
												</button>`
