...
$me = $api->me();
$my_save_album = $api->getMySavedAlbums();

$countPerpage = 50;
$total = $my_save_album->total;
$totalPage = ceil($total / $countPerpage);
$my_save_album = $api->getMySavedAlbums([
    'limit' => $countPerpage,
]);
$bg_box_savealbum = get_option('_bg_box_savealbum');
...
<style>
    ...
    .bg-box {
        background-color: <?= $bg_box_savealbum ? $bg_box_savealbum : '#fff'; ?>;
    }
...
<div class="flex flex-1 flex-col bg-box gap-2 sm:-mb-4" style="max-width: inherit;">
    ...
    <div class="flex flex-1 flex-row">
        <div class="grid grid-cols-2 lg:grid-cols-4 mx-auto gap-4" id="frame">
            <?php foreach ($my_save_album->items as $saved_album) : ?>
                <a href="#data_playlist" onclick="ShowSavedAlbum(this)" data-name="<?= $saved_album->album->name; ?>" data-id="<?= $saved_album->album->id; ?>" class="flex flex-col hover:bg-transparent focus:bg-transparent">
                    <img src="<?= $saved_album->album->images[0]->url; ?>" class="lg:w-40 md:h-40 lg:w-40 md:h-40 sm:w-36 sm:h-36" />
                    <p class="text-center"><?= $saved_album->album->name; ?></p>
                </a>
            <?php endforeach; ?>
        </div>
        ...
        <div class="flex flex-1 flex-col justify-center items-center">
            <nav aria-label="Page navigation example">
                <ul class="pagination flex flex-1 flex-row justify-center gap-2">
                    <?php if ($my_save_album->previous != null) : ?>
                        <li class="page-item">
                            <button class="page-link" data-page="<?= $my_save_album->previous ?>">Previous</button>
                        </li>
                    <?php endif; ?>
                    ...
                    <?php endfor; ?>
                    <?php if ($my_save_album->next != null) : ?>
                        <li class="page-item">
                            <button class="page-link" data-page="<?= $my_save_album->next ?>">Next</button>
                        </li>
                    <?php endif; ?>
                </ul>
            </nav>
