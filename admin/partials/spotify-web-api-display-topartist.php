...
$me = $api->me();
$artist_top = $api->getMyTop('artists');
$countPerpage = 50;
$total = $artist_top->total;
$totalPage = ceil($total / $countPerpage);
$mytopartist = $api->getMyTop('artists', [
    'limit' => $countPerpage,
]);
$number_grid_topartist = get_option('_number_grid_topartist');
$bg_box_topartist = get_option('_bg_box_topartist');
...
<style>
    ...
    .bg-box {
        background-color: <?= $bg_box_topartist ? $bg_box_topartist : '#fff'; ?>;
...
<div class="flex flex-1 flex-col bg-box gap-2 sm:-mb-4" style="max-width: inherit;">
    ...
    <div class="flex flex-1 flex-row">
        <div class="grid grid-cols-2 lg:grid-cols-<?= $number_grid_topartist ? $number_grid_topartist : '4'; ?> mx-auto gap-4" id="frame">
            <?php foreach ($mytopartist->items as $top_artist) : ?>
                <a href="#data_playlist" onclick="ShowAnyTracks(this)" data-name="<?= $top_artist->name; ?>" data-id="<?= $top_artist->id; ?>" class="flex flex-col hover:bg-transparent focus:bg-transparent">
                    <img src="<?= $top_artist->images[0]->url; ?>" class="lg:w-40 md:h-40 lg:w-40 md:h-40 sm:w-36 sm:h-36" />
                    <p class="text-center"><?= $top_artist->name; ?></p>
                </a>
            <?php endforeach; ?>
        </div>
        ...
        <div class="flex flex-1 flex-col justify-center items-center">
            <nav aria-label="Page navigation example">
                <ul class="pagination flex flex-1 flex-row justify-center gap-2">
                    <?php if ($mytopartist->previous != null) : ?>
                        <li class="page-item">
                            <button class="page-link" data-page="<?= $mytopartist->previous ?>">Previous</button>
                        </li>
                    <?php endif; ?>
                    ...
                    <?php endfor; ?>
                    <?php if ($mytopartist->next != null) : ?>
                        <li class="page-item">
                            <button class="page-link" data-page="<?= $mytopartist->next ?>">Next</button>
                        </li>
                    <?php endif; ?>
                </ul>
            </nav>
...
<script>
  ...
  <div class="flex flex-1 flex-col gap-5 p-4 shadow-md">
    <div class="flex flex-1 justify-center items-center gap-4">
                <img src="${res.artist_data.images[0].url}" class="w-14 h-14 lg:w-36 lg:h-36" />
                <div class="flex flex-1 flex-col">
                <h1 class="text-md lg:text-xl font-bold">${res.artist_data.name}</h1>
        ...
        ${res.album_artist.length > 0 ?
          ...
          `<h1 class="font-bold">Albums</h1>
            ...
            ${res.album_artist.map(item => {
                ...
                ${res.compalation_artist.length > 0 ? `
    ...
    jQuery(document).ready(function($) {
        ...
        $('#search_top_artist').keyup(function(event) {
            event.preventDefault();
            var search_top_artist = $('#search_top_artist').val();
            var data_array = [];
            var result_search = [];
            var fileter_iframe = [];
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'search_top_artist',
                    search_top: search_top_artist
                },
                success: function(data) {
                    let search = JSON.parse(data);
                    console.log(search);
                    // get all string with regexp
                    if (search_top_artist !== '') {
                        ...
                        // search
                        let obj_data = data_array.find(x => x.name.toLowerCase().includes(search_top_artist.toLowerCase()));
                        result_search.push(obj_data);
                        $.each(result_search, function(index, val) {
                            if (val) {
                                let frame = `<button type="button" onclick="PlayTrack(this)" data-id="${val.id}" class="flex flex-col hover:bg-transparent focus:bg-transparent text-center">
